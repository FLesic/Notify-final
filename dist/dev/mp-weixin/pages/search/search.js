...
var _search = _interopRequireDefault(__webpack_require__(/*! ./pages/search/search.vue */ 157));
...
var _default = {
  ...
  methods: {
    SearchNoticeByKeyword: function SearchNoticeByKeyword() {
      ...
      // 		page:this.page,
      // 	};
      // 	request(`notice/search/keyword`,reqdata)
      // 	.then((data)=>{
    ...
    search: function search() {
      ...
      } else if (this.inputValue.replace(/ /g, "") == "") {
        uni.showModal({
          title: '搜索内容不能为空'
        });
      } else {
      ...
      if (this.inputValue !== '' && this.inputValue.replace(/ /g, "") !== "") {
        uni.navigateTo({
          url: '../searchto/searchto?keyword=' + JSON.stringify(this.inputValue)
        });
      }
    },
    //清空历史记录
    ...
    throughHistorySearch: function throughHistorySearch(item) {
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 2:
              ...
              if (list[1].data) {
                _this.searchHistoryList = JSON.parse(list[1].data);
              }
              uni.$on('transmit', function (data) {
                console.log('搜索是' + data.msg);
                this.inputValue = data.msg;
              });
            case 6:
            case "end":
